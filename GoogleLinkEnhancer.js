    ...
    class GLE {
        ...
        #isLinkTooFar(link, venue) {
            if (link.loc) {
                ...
                const distance = _a.#distanceBetweenPoints(linkPt, venuePt);
                    ...
                    for (const venue of this.sdk.DataModel.Venues.getAll()) {
                        ...
                        for (const provID of venue.externalProviderIds) {
                            ...
                            if (linkInfo?.count > 1) {
                                ...
                                // })];
                                const features = [
                                    _a.isPointVenue(venue) ? turf.point(geometry.coordinates, {
                                ...
                                const lineStart = turf.centroid(geometry);
                        ...
                        Promise.all(promises).then(results => {
                            ...
                            if (strokeColor) {
                                ...
                                // const geometry = venue.isPoint() ? venue.geometry.getCentroid() : venue.geometry.clone();
                                const feature = _a.isPointVenue(venue) ? turf.point(venue.geometry.coordinates, {
                                    styleName: "placeStyle",
                                    style: style
                                }) : turf.polygon(venue.geometry.coordinates, {
                                    styleName: "placeStyle",
                                    style: style
                                });
        ...
        async #addPoint(id) {
            ...
            if (link) {
                if (!link.notFound) {
                    ...
                    const ext = _a.#getOLMapExtent(this.sdk);
                    ...
                    if (splits) {
                        ...
                        for (const split of splits) {
                            for (const component of split.components) {
                                if (component.x === placePt.x && component.y === placePt.y)
                                    splitPoints = split;
                            }
                            ;
                        }
                        ...
                        let distance = _a.#distanceBetweenPoints(poiPt, placePt);
                        let unitConversion;
                        let unit1;
                        let unit2;
