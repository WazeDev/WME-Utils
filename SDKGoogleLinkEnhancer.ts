...
const SDKGoogleLinkEnhancer = (() => {
    ...
    class GLE {
        ...
        linkCache;
        #enabled = false;
        static #mapLayer = "Google Link Enhancements.";
        ...
        constructor(sdk: WmeSDK, trf: typeof turf) {
            let msg = "";
            if (!sdk) {
        ...
        static isPointVenue(venue: Venue) {
                            ...
                            if (selection?.objectType === "venue") {
                                ...
                                if (venue && this.#isLinkTooFar(link, venue)) {
                                    $extProvElem
                                        ...
                                        .attr(
                                            "title",
                                            this.strings.tooFar.replace("{0}", this.distanceLimit.toString())
                                        );
                                } else {
        ...
        static #getExistingLinks(sdk: WmeSDK | undefined = undefined) {
            if (!sdk) {
        ...
        static #getIdFromElement($el) {
            ...
            return W.selectionManager.getSelectedDataModelObjects()[0].getExternalProviderIDs()[providerIndex]
                ?.attributes.uuid;
        }

        #addHoverEvent($el) {
            $el.hover(
                () => this.#addPoint(GLE.#getIdFromElement($el)),
                () => this.#destroyPoint()
            );
        }
                    ...
                    switch (status) {
                        case google.maps.places.PlacesServiceStatus.OK: {
                            ...
                            if (result.business_status === google.maps.places.BusinessStatus.CLOSED_PERMANENTLY) {
                                ...
                                result.business_status === google.maps.places.BusinessStatus.CLOSED_TEMPORARILY
                            ) {
